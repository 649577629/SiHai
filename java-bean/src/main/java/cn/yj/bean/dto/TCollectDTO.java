...
@NoArgsConstructor
@AllArgsConstructor
public class TCollectDTO implements Serializable {
	...
	/** 表_收藏 BO 转换 DTO */
	public static TCollectDTO toDTO(TCollectBO bo) {
		if (bo != null) {
			return TCollectDTO.builder()
					.keyID(bo.getKeyID())
					.userID(bo.getUserID())
	...
	/** 表_收藏 BO 数组 转换 DTO List */
	public static List<TCollectDTO> toDTO(TCollectBO[] bos) {
		List<TCollectDTO> dtos = new ArrayList<>();
	...
	/** 表_收藏 BO List 转换 DTO List */
	public static List<TCollectDTO> toDTO(List<TCollectBO> bos) {
		List<TCollectDTO> dtos = new ArrayList<>();
		for (TCollectBO bo : bos) {
			dtos.add(toDTO(bo));
		}
	...
	/** 表_收藏 DTO 转换 BO */
	public static TCollectBO toBO(TCollectDTO dto) {
		if (dto != null) {
	...
	/** 表_收藏 DTO List 转换 BO List */
	public static List<TCollectBO> toBO(List<TCollectDTO> dtos) {
		List<TCollectBO> bos = new ArrayList<>();
		for (TCollectDTO dto : dtos) {
			bos.add(toBO(dto));
		}
		return bos;
	}

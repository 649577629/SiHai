...
import cn.yj.bean.dto.TAdminDTO;
...
public class TAdminRestController {
	 ...
	 */
	@ApiOperation(value = "新增 表_管理员", notes = "入参 TAdminDTO 表_管理员 对象 进行新增则无需传入主键")
	@PostMapping(value = "/insert")
	public ResultMessage insert(
			@ApiParam(value = "表_管理员 DTO", required = true) TAdminDTO dto
	) throws BusinessException {
		return tAdminService.insert(dto.toBO());
	}
	 ...
	 * @return code=200 成功 | code=1 失败
	 */
	@ApiOperation(value = "修改 表_管理员", notes = "入参 TAdminDTO 表_管理员 对象 进行修改则传入对应主键 )")
	@PostMapping(value = "/update")
	public ResultMessage update(
			@ApiParam(value = "表_管理员 DTO", required = true) TAdminDTO dto
	) throws BusinessException {
		return tAdminService.update(dto.toBO());
	}
	...
	public ResultMessage deleteById(
		...
		tAdminService.deleteById(keyID);
	...
	public ResultMessage getByID(
		...
		return new ResultMessage(TAdminDTO.toDTO(tAdminService.getByID(keyID)));
	}
	...
	public ResultMessage queryBy(
		...
		return new ResultMessage(tAdminService.queryBy(currentPage, eachPageNumber));
	}

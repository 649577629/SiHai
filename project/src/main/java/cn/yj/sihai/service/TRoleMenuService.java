...
import cn.yj.sihai.mapper.TRoleMenuMapper;
...
public class TRoleMenuService {
	...
	public ResultMessage insert(
		...
		try {
			// TODO 需添加步骤赋值新增ID
			int result = tRoleMenuMapper.insert(bo.toPO());
			if (result > 0) {
	...
	public ResultMessage update(
		...
		try {
			int result = tRoleMenuMapper.update(bo.toPO());
			if (result > 0) {
	...
	public int deleteById(
		...
		try {
			return tRoleMenuMapper.deleteById(keyID);
		} catch (Exception e) {
	 ...
	 * @return 表_角色菜单关系 对象
	 */
	public TRoleMenuBO getByID(
			String keyID
	) throws BusinessException {
		try {
			return TRoleMenuBO.toBO(tRoleMenuMapper.getByID(keyID));
		} catch (Exception e) {
	 ...
	 * @return 表_角色菜单关系 集合
	 */
	public List<TRoleMenuBO> queryBy(
			int currentPage,
			int eachPageNumber
	) throws BusinessException {
		try {
			return TRoleMenuBO.toBO(tRoleMenuMapper.queryBy(currentPage - 1, eachPageNumber));
		} catch (Exception e) {

...
import cn.yj.bean.dto.TMenuDTO;
...
/** 表_菜单 RestController */
@Api(tags = "表_菜单 RestController")
@RestController
@RequestMapping(value = "/Menu", produces = MediaType.APPLICATION_JSON_VALUE)
public class TMenuRestController {
	 ...
	 */
	@ApiOperation(value = "新增 表_菜单", notes = "入参 TMenuDTO 表_菜单 对象 进行新增则无需传入主键")
	@PostMapping(value = "/insert")
	public ResultMessage insert(
			@ApiParam(value = "表_菜单 DTO", required = true) TMenuDTO dto
	) throws BusinessException {
		return tMenuService.insert(dto.toBO());
	}
	 ...
	 * @return code=200 成功 | code=1 失败
	 */
	@ApiOperation(value = "修改 表_菜单", notes = "入参 TMenuDTO 表_菜单 对象 进行修改则传入对应主键 )")
	@PostMapping(value = "/update")
	public ResultMessage update(
			@ApiParam(value = "表_菜单 DTO", required = true) TMenuDTO dto
	) throws BusinessException {
		return tMenuService.update(dto.toBO());
	}
	 ...
	 */
	@ApiOperation(value = "根据主键删除 表_菜单 对象", notes = "入参 keyID 需删除主键值 进行删除操作")
	 ...
	 * @return 表_菜单 对象
	 */
	@ApiOperation(value = "根据主键查询 表_菜单 对象", notes = "入参 keyID 需查询主键 进行查询操作")
	@GetMapping(value = "/getByID/{keyID}")
	public ResultMessage getByID(
			@ApiParam(value = "主键值", required = true) @PathVariable("keyID") String keyID
	) throws BusinessException {
		return new ResultMessage(TMenuDTO.toDTO(tMenuService.getByID(keyID)));
	}
	 ...
	 * @return 表_菜单 集合
	 */
	@ApiOperation(value = "分页查询 表_菜单 集合 ", notes = "入参 currentPage 当前页 | eachPageNumber 每页条数 进行分页查询操作")
	...
	public ResultMessage queryBy(
		...
		return new ResultMessage(tMenuService.queryBy(currentPage, eachPageNumber));
	}

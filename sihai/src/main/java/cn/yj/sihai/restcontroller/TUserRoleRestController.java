...
import cn.yj.bean.dto.TUserRoleDTO;
...
/** 表_用户角色关系 RestController */
@Api(tags = "表_用户角色关系 RestController")
@RestController
@RequestMapping(value = "/UserRole", produces = MediaType.APPLICATION_JSON_VALUE)
public class TUserRoleRestController {
	 ...
	 * 新增 表_用户角色关系
	 * 
	 * @param dto 表_用户角色关系 DTO
	 * @return code=200 成功 | code=1 失败
	 */
	@ApiOperation(value = "新增 表_用户角色关系", notes = "入参 TUserRoleDTO 表_用户角色关系 对象 进行新增则无需传入主键")
	@PostMapping(value = "/insert")
	public ResultMessage insert(
			@ApiParam(value = "表_用户角色关系 DTO", required = true) TUserRoleDTO dto
	) throws BusinessException {
		return tUserRoleService.insert(dto.toBO());
	}
	 ...
	 * @return code=200 成功 | code=1 失败
	 */
	@ApiOperation(value = "修改 表_用户角色关系", notes = "入参 TUserRoleDTO 表_用户角色关系 对象 进行修改则传入对应主键 )")
	@PostMapping(value = "/update")
	public ResultMessage update(
			@ApiParam(value = "表_用户角色关系 DTO", required = true) TUserRoleDTO dto
	) throws BusinessException {
		return tUserRoleService.update(dto.toBO());
	}
	 ...
	 */
	@ApiOperation(value = "根据主键删除 表_用户角色关系 对象", notes = "入参 keyID 需删除主键值 进行删除操作")
	...
	public ResultMessage deleteById(
		...
		tUserRoleService.deleteById(keyID);
	 ...
	 * @return 表_用户角色关系 对象
	 */
	@ApiOperation(value = "根据主键查询 表_用户角色关系 对象", notes = "入参 keyID 需查询主键 进行查询操作")
	@GetMapping(value = "/getByID/{keyID}")
	public ResultMessage getByID(
			@ApiParam(value = "主键值", required = true) @PathVariable("keyID") String keyID
	) throws BusinessException {
		return new ResultMessage(TUserRoleDTO.toDTO(tUserRoleService.getByID(keyID)));
	}
	 ...
	 * @return 表_用户角色关系 集合
	 */
	@ApiOperation(value = "分页查询 表_用户角色关系 集合 ", notes = "入参 currentPage 当前页 | eachPageNumber 每页条数 进行分页查询操作")
	...
	public ResultMessage queryBy(
		...
		return new ResultMessage(tUserRoleService.queryBy(currentPage, eachPageNumber));
	}

...
import cn.yj.sihai.mapper.TCompanyMapper;
...
public class TCompanyService {
	...
	public ResultMessage insert(
		...
		try {
			// TODO 需添加步骤赋值新增ID
			int result = tCompanyMapper.insert(bo.toPO());
			if (result > 0) {
	...
	public ResultMessage update(
		...
		try {
			int result = tCompanyMapper.update(bo.toPO());
			if (result > 0) {
	...
	public int deleteById(
		...
		try {
			return tCompanyMapper.deleteById(keyID);
		} catch (Exception e) {
	 ...
	 * @return 表_管理员 对象
	 */
	public TCompanyBO getByID(
			String keyID
	) throws BusinessException {
		try {
			return TCompanyBO.toBO(tCompanyMapper.getByID(keyID));
		} catch (Exception e) {
	 ...
	 * @return 表_管理员 集合
	 */
	public List<TCompanyBO> queryBy(
			int currentPage,
			int eachPageNumber
	) throws BusinessException {
		try {
			return TCompanyBO.toBO(tCompanyMapper.queryBy(currentPage - 1, eachPageNumber));
		} catch (Exception e) {
